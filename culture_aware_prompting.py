...
def main(device_id, model_path="", lora_name="", culture_specific=False, run_id=0, self_alignment=False, question_path = "./data/wvs_questions.json"):
    ...
    for country, culture in country2culture_dict.items():
        ...
        if "wvs_questions" in question_path:
            if self_alignment:
                save_path = f"{save_dir}/wvs_result_culture_aware_self_alignment_{country}_{run_id}.json"
            else:
                save_path = f"{save_dir}/wvs_result_culture_aware_{country}_{run_id}.json"
        else:
            seed_setting = question_path.split("_")[-2]
            save_path = f"{save_dir}/generated_from_{seed_setting}_seed_result_culture_aware_{country}.json"
        result = []
